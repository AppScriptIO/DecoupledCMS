...
export function oidcInteractionEntrypoint({ // interaction entrypoint
    openIdConnectServer
}) { return async (context, next) => { // redirect to a specific interaction (e.g. login, confirm, etc.) depending on the interactionDetails provided by the oidc checking of the /authorize request.
    ...
    if(pathArray[0] == 'interaction' && !pathArray[2]) {
        ...
        if (details.interaction.error === 'login_required') { // provide/serve login html view
...
export function oidcInteractionLogin({
    openIdConnectServer
    ...
    if(pathArray[0] == 'interaction' && pathArray[2] == 'login') {
        ...
        const result = {
            login: { // authentication/login prompt got resolved, omit if no authentication happened, i.e. the user cancelled
