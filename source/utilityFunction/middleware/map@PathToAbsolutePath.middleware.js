...
function _default() {
  return async (context, next) => {
    let path = context.path; // path = path.replace(/^\/|\/$/g, '') // remove first and last slash

    let pathArray = path.split('/').filter(item => item); // split path and remove empty values

    let firstURLPart = pathArray[0]; // let lastIndexPosition = (path.indexOf("/") == -1) ? path.length : path.indexOf("/");
    ...
    context.relativeAtPathName = relativeAtPathName; // change path if @ path is mapped
