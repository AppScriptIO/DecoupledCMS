...
exports.oidcInteractionEntrypoint = oidcInteractionEntrypoint;
exports.oidcInteractionLogin = oidcInteractionLogin;
exports.oidcInteractionConfirm = oidcInteractionConfirm;
...
function oidcInteractionLogin({
  ...
  return async (context, next) => {
    ...
    if (pathArray[0] == 'interaction' && pathArray[2] == 'login') {
      ...
      const password = context.request.body.password; // check username and password & get user unique id // TODO: check credentials

      const accountId = username; // results should be an object passed to openIdConnectServer.interactionFinished with some or all the following properties
      ...
      const result = {
        login: {
          ...
          // acr value for the authentication 
          amr: ['pwd'],
          remember: !!context.request.body.remember,
          // true if provider should use a persistent cookie rather than a session one
