...
var _default = {
  // Model functions required by node-auth2-server See https://github.com/oauthjs/node-oauth2-server for specification
  ...
  getClient: async (clientId, clientSecret) => {
    ...
    const connection = _ApplicationClass.default.rethinkdbConnection;
    let dbFilterObject = clientSecret ? {
      key: clientId,
    ...
    if (!client) return null; // return in required format.

    return {
      id: client.key,
      redirectUris: [client.redirectUri],
      grants: client.grantType // Client-specific lifetime of generated refresh tokens in seconds.
  ...
  getAccessToken: async accessToken => {
    ...
    const connection = _ApplicationClass.default.rethinkdbConnection;
    let tokenData = await getTableDocument.instance['oAuth_token'](connection, {
      type: 'accessToken',
      token: accessToken
    });
    if (!tokenData) return null;
    let client = await getTableDocument.instance['oAuth_client'](connection, {
      key: tokenData.clientId
    });
    let user = await getTableDocument.instance['oAuth_user'](connection, {
  ...
  getRefreshToken: async refreshToken => {
    ...
    return {
      ...
      // with 'id' property
      user: {
        username: user.key // with 'username' property
  ...
  saveToken: async (token, client, user) => {
    ...
    let userId = user.username; // TODO: set expiration / TTL - https://groups.google.com/forum/#!topic/rethinkdb/tFSiG5Ex1KE for rethinkdb underdevelopment.
  ...
  revokeToken: async token => {
    ...
    } = await _rethinkdb.default.db('webappSetting').table('oAuth_token').filter(filterObject).delete().run(connection);
  ...
  verifyScope: async (accessToken, scope) => {
    console.log('verifyScope function'); //no scope declared for the resource, free to access

    if (!scope) return true;
    if (!accessToken.scope) return false;
    validScopes = scope.split(',').map(s => s.trim());
    scopes = accessToken.scope.split(',').map(s => s.trim()); //check if at least one of the scopes granted to the access token are allowed to access the resource
